...
with open('input/pruebas.txt', 'r', encoding = 'utf-8') as msgFile:
...
for message in messages:
	...
	annResult = annotator.annotator(message, '../annotator_pkg/lang_data/2_defeatVirus_nodes.json')
	...
	while choice == 'n' or choice == 't':
		...
		if choice == 'n':
			print('\n')
			annInputs = queryForNode('../annotator_pkg/lang_data/2_defeatVirus_nodes.json', '../annotator_pkg/lang_data/defeatVirus_ann_scheme.json')
			writeToNodeFile(annInputs, '../annotator_pkg/lang_data/2_defeatVirus_nodes.json')
			annStringToDisplay = annotator.annotator(message, '../annotator_pkg/lang_data/2_defeatVirus_nodes.json')[1]
		...
		if choice == 't':
			...
			annString = annResult[0]
			annStringToDisplay = annResult[1]
			queryAndAddTrain(annString, '../classifier_pkg/defeatVirus.train.txt', '../annotator_pkg/lang_data/defeatVirus_ann_scheme.json')
			print('\n')
			model = classifier.loadModel('../classifier_pkg/defeatVirus.train.txt')
			classifierResult = classifier.classify(model, annString)
